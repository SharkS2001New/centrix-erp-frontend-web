...
export function salesOrganizationFormFromApi(res) {
  ...
  return {
    ...
    show_full_package_uom_on_documents: sales.show_full_package_uom_on_documents === true,
...
export function salesOrganizationPayloadFromForm(form, capabilities = null) {
  ...
  const {
    ...
    show_full_package_uom_on_documents: _showFullPackageUomOnDocuments,
 ...
 *
 * `/erp/capabilities` resolves the effective flag server-side for orgs on classic POS.
 */
...
export function getCheckoutPaymentConfig(moduleSettings, options = {}) {
  ...
  return {
    ...
    // External POS / Create order (checkoutContext "pos"): never allow partial tenders.
    // Credit is only via the credit customer field and is always fully unpaid.
    // Backoffice Collect payment (order_payment): gated by allow_credit_pay_now only.
    allowPartialPayment:
