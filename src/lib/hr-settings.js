...
export function formatAttendanceSource(source, sourceLabel) {
  ...
  if (source === "hr_applied") return "Applied by HR manually";
...
export function hrPayrollFormFromApi(res) {
  ...
  return {
    ...
    hikvision_agent_poll_minutes: String(hr.hikvision_agent_poll_minutes ?? 10),
  };
}
...
export function hrPayrollPayloadFromForm(form) {
  return {
    ...
    hikvision_agent_poll_minutes: Math.max(1, Math.min(60, Number(form.hikvision_agent_poll_minutes) || 10)),
  };
}
