...
export function PlatformMailboxPanel() {
  ...
  options={[{ value: "", label: "All sent types" }, { value: "subscription_renewal_reminder", label: Renewal reminders
              {mailStats?.renewal_reminders?.all_time != null
                ? ` (${mailStats.renewal_reminders.all_time})`
                : ""} }, { value: "two_factor", label: 2FA codes
              {mailStats?.two_factor?.all_time != null ? ` (${mailStats.two_factor.all_time})` : ""} }, { value: "email_verification", label: Email verification
              {mailStats?.email_verification?.all_time != null
                ? ` (${mailStats.email_verification.all_time})`
                : ""} }, { value: "subscription_renewal_reminder_test", label: "Renewal tests" }, { value: "test", label: "SMTP tests" }]}
/>
        ) : null}
  ...
  options={organizations.map((org) => ({ value: org.id, label: org.org_name }))}
/>
                </label>
                <label className="block text-sm">
...
/>
  ...
  options={composeTemplates.map((tpl) => ({ value: tpl.id, label: tpl.name }))}
/>
                </label>
                <button
                  type="button"
