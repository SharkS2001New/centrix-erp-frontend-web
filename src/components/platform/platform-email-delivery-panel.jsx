      ...
      {(activeEmailTab === "smtp" || activeEmailTab === "imap") && (
        <div className="theme-panel flex flex-wrap items-center gap-2 rounded-xl border px-4 py-3 shadow-sm">
          ...
          <SearchableSelect
            ...
            options={(form.accounts || []).map((account) => ({
              value: account.id,
              label: `${mailboxAccountLabel(account)}${account.is_default ? " (default)" : ""}`,
            }))}
          />
          <button
            type="button"
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <label className="block text-sm">
              ...
              <SearchableSelect
                ...
                options={PLATFORM_INVOICE_DESIGN_TEMPLATES.map((tpl) => ({
                  value: tpl.id,
                  label: tpl.label,
                }))}
              />
            ...
            <label className="block text-sm">
              ...
              <SearchableSelect
                ...
                placeholder="— Use design above / plan template —"
                options={[
                  { value: "", label: "— Use design above / plan template —" },
                  ...invoiceSavedTemplates.map((tpl) => ({
                    value: tpl.id,
                    label: `${tpl.name}${tpl.template_id ? ` (${tpl.template_id})` : ""}`,
                  })),
                ]}
              />
              <span className="mt-1 block text-[11px] text-slate-500">
