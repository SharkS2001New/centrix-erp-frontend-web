...
export function EndOfDayReportScreen() {
  ...
  function handleExportCsv() {
    if (!report) return;
    const lines = [
      ["End of Day Report", branchName, cashierName ?? "All cashiers", saleDate].join(","),
      ["Gross sales", summary.gross_sales].join(","),
      ...(discountsEnabled ? [["Total discounts", summary.total_discounts].join(",")] : []),
      ["Net sales", summary.net_sales].join(","),
      ["Transactions", summary.transactions].join(","),
      ["Cash", payments.cash].join(","),
      ["M-Pesa", payments.mpesa].join(","),
      ["Bank", payments.bank].join(","),
    ];
    const blob = new Blob([lines.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `eod-report-${saleDate}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }

  return (
    <div className="-m-6 min-h-[calc(100%+3rem)] bg-slate-50 p-6 text-black md:-m-8 md:min-h-[calc(100%+4rem)] md:p-8">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button
            ...
            onClick={handlePrint}
            disabled={!report}
            className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-black hover:bg-slate-50 disabled:opacity-50"
          >
            Print report
          </button>
          <button
