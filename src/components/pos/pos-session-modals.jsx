...
export function ZReportModal({
  ...
  async function handlePrint({ wipeEvenIfPrintFails = false } = {}) {
    if (printing) return;
    setPrinting(true);
    let printOk = false;
    try {
    ...
    if (!printOk && !wipeEvenIfPrintFails) return;
    try {
      await onPrinted?.();
    } catch (err) {
  ...
  async function handleCloseAndFinish() {
    await handlePrint({ wipeEvenIfPrintFails: true });
  }
  ...
  return (
    <PosSessionDialogShell
      ...
      subtitle={
          signOutAfterFinish
          ? "Print Z or Close session to clear this till’s local sales, then sign out. The next session starts at Cash Sales #1."
        ...
        report?.sales || report?.expected_cash != null ? (
          ...
          <div className="flex justify-end">
            <button
              type="button"
              disabled={printing}
              onClick={() => void (signOutAfterFinish ? handleCloseAndFinish() : onClose())}
              className="rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50"
            >
              {signOutAfterFinish ? "Close session" : closeLabel}
            </button>
          </div>
        )
      }
