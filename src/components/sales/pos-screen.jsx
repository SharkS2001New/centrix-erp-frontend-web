...
function usesPosLocalDraftLineEdits(cart) {
  return Boolean(
    isPreviousOrderEditSession(cart) ||
      ...
      (cart?.id != null && !isServerPosCartId(cart.id)),
  );
}
...
async function resolveNextPosTicketForWorkspace(
  ...
  // Outbox = real on-device issues. Meta alone can be a phantom server watermark (#1
  // "issued" with no sale) that must not skip unused Cash Sales #1.
  const outboxMax = await peekOutboxPosTicketMax(null, floatSessionId).catch(() => null);
  ...
  // Floor from real issues only (outbox / session). Do not use meta-only phantoms.
  const localFloorNext =
    outboxMax != null && Number(outboxMax) > 0
      ? Number(outboxMax) + 1
      : sessionNext != null && Number(sessionNext) > 1
        ? localNext
        : null;

  // Fresh blank till with no session receipts and no pending outbox: Cash Sales #1.
  // Heal poisoned meta so offline allocate also starts at #1 (not #2).
  if (emptyFresh && Number(sessionNext) === 1 && !(outboxMax > 0)) {
    if (localNext != null && Number(localNext) > 1) {
      void seedLocalPosTicketSeq(0, null, floatSessionId, { force: true }).catch(() => {});
    }
    return 1;
  }
  ...
  // Systematic next-ticket order (must stay consecutive — no jumping):
  // Prefer session-derived next; raise with outbox / local floor when tickets exist.
  if (sessionNext != null && Number(sessionNext) > 1) {
    const raised = [sessionNext, localFloorNext, localNext, issuedPosMax != null ? Number(issuedPosMax) + 1 : null]
      .filter((n) => n != null && Number(n) > 0)
      .map((n) => Number(n));
    return raised.length ? Math.max(...raised) : Number(sessionNext);
  }
  if (outboxMax != null && Number(outboxMax) > 0) {
    return Number(outboxMax) + 1;
  }
  return 1;
}
...
export function PosScreen({ standalone = false }) {
  ...
  /** True while hold/payment/etc. own the keyboard — Scan must not steal focus. */
  const posOverlayBlocksScanRef = useRef(false);
  /**
  ...
  async function confirmWipeLocalPosDataIfNeeded() {
    ...
    return confirm({
      ...
      message:
        ...
        "that have not synced to the server. Printing Z will wipe them from this device. " +
  ...
  /** Snapshot of the line being swapped — survives TemporaryCart / optimistic id remints. */
  const replaceTargetSnapshotRef = useRef(null);
  ...
  /**
   * Synchronous gate armed the instant Delete / Add starts — before the first await —
   * so fast Delete→scan→Enter cannot queue a twin add while removeSelectedLines is
   * still spinning up the blocking overlay.
   */
  const cartLineMutationGateRef = useRef(false);
  ...
  /**
   * Search/barcode Enter can land on qty after focus handoff. Swallow only while
   * that same Enter key is still down (or a short grace after release / async park).
   */
  const suppressNextEntryQtyEnterRef = useRef(false);
  const suppressNextEntryQtyEnterClearTimerRef = useRef(null);
  const suppressNextEntryQtyEnterKeyupRef = useRef(null);
  ...
  const loadCompletedPosOrders = useCallback(async () => {
    ...
    try {
      ...
      // Synced outbox rows stay in IndexedDB (not deleted). Mirror them into browse
      // so upload does not blank Cash Sales # between pending → server list.
      let localSyncedOrders = [];
      try {
        localSyncedOrders = await listLocalSyncedSalesForBrowse();
      } catch {
        localSyncedOrders = [];
      }
      ...
      const localSyncedBrowseKeys = new Set(
        localSyncedOrders
          .map((row) => resolvePosSessionTicketNumber(row))
          .filter((n) => n != null && !offlineBrowseKeys.has(String(n)))
          .map(String),
      );
      ...
      setEditOrderNo((current) => {
        ...
        if (onFreshNewOrder) {
          ...
          // Always keep the on-screen next (current) — a short session list / stale
          // IndexedDB seq must not rewind New Order 32 → 14 when the link drops.
          ...
          const currentNum = Number(String(current ?? "").trim());
          const candidates = [nextScoped, liveNext, currentNum]
            .map((n) => Number(n))
            .filter((n) => Number.isFinite(n) && n > 0);
  ...
  const posSearchSuspended =
    posOverlayBlocksScan || preparingNextOpen || Boolean(autoHeldBusy);
  ...
  const materializeOfflineCartOnServer = useCallback(
    async (localCart, options = {}) => {
      ...
      const task = (async () => {
        // Previous-order edits must restore onto the original sale — never POST a blank
        // TemporaryCart (that drops superseded_sale_id and mints a new order on checkout).
        if (
          localCart.held_order_num &&
          localCart.superseded_sale_id &&
          !localCart.offline_client_sale_uuid
        ) {
          const saleId = Number(localCart.superseded_sale_id);
          const restored = await apiRequest(`/sales/orders/${saleId}/restore-to-cart`, {
            method: "POST",
            body: { replace: true },
            ...POS_CART_REQUEST,
          ...
          let serverCart = restored;
          if (linePayload.length || Number(localCart.order_discount ?? 0)) {
            serverCart = await apiRequest(`/sales/carts/${restored.id}/lines`, {
              method: "PUT",
              body: {
                lines: linePayload,
          ...
          const withMarkers = {
            ...(normalizeCartResponse(serverCart) ?? serverCart),
            held_order_num: localCart.held_order_num,
            superseded_sale_id: saleId,
          };
          if (Array.isArray(localCart.payment_adjustments)) {
            withMarkers.payment_adjustments = localCart.payment_adjustments;
          }
          void clearLocalPosCart().catch(() => {});
          if (applyToWorkspace) {
            cartRef.current = withMarkers;
            setCart(withMarkers);
          }
          return withMarkers;
        }
  ...
  function armCartLineMutationGate() {
  ...
  /** True while an overlay/commit is actually running (excludes Delete-key pre-arm). */
  function isCartLineSaveBusy() {
    return (
      lineSaveOverlayDepthRef.current > 0 ||
      cartCommitPendingRef.current > 0 ||
  ...
  /** Drop stale `_optimistic` paint flags once the commit chain is idle (F10 must not soft-fail). */
  function adoptCartWithoutOptimisticFlags(cartNow) {
    if (!cartHasOptimisticLines(cartNow)) return cartNow;
    const cleaned = {
      ...cartNow,
      lines: (cartNow.lines ?? []).map((line) => {
        if (!line?._optimistic) return line;
  ...
  function clearSuppressNextEntryQtyEnter() {
    ...
    if (suppressNextEntryQtyEnterKeyupRef.current) {
      window.removeEventListener("keyup", suppressNextEntryQtyEnterKeyupRef.current, true);
      suppressNextEntryQtyEnterKeyupRef.current = null;
    }
  }

  /**
   * Swallow qty/swap Enter only while the Search/barcode Enter key is still down
   * (same physical key leaking after focus). Keyup + short grace clears so a
   * second Enter can add default qty. Async barcode park after keyup uses grace only.
   */
  function armSuppressNextEntryQtyEnter() {
    clearSuppressNextEntryQtyEnter();
    suppressNextEntryQtyEnterRef.current = true;
    const startGraceClear = (ms) => {
      if (suppressNextEntryQtyEnterClearTimerRef.current != null) {
        window.clearTimeout(suppressNextEntryQtyEnterClearTimerRef.current);
      }
    ...
    if (!enterKeyHeldRef.current) {
      // Originating Enter already released (barcode await) — do not block confirm.
      startGraceClear(80);
      return;
    }
    const onKeyUp = (e) => {
    ...
    suppressNextEntryQtyEnterKeyupRef.current = onKeyUp;
  ...
  function scheduleFocusEntryQty(opts = {}) {
    ...
    focusSearchAfterAdd.current = false;
    // Do not default a time window here — that swallowed a fast confirm Enter.
    // Keyboard leak uses suppressNextEntryQtyEnterRef; Add twins use handleAddLine.
    const ignoreEnterMs = opts.ignoreEnterMs ?? 0;
    if (ignoreEnterMs > 0) {
  ...
  // Also raise the on-device Cash Sales counter to the UI next so a blank New Order 32
  // does not allocate from a stale IndexedDB seq (e.g. print #14) after the drop.
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const current = cartRef.current;
        const uiNext = Math.max(
          Number(editOrderNo) || 0,
          Number(offlineNextPosOrderNum) || 0,
          Number(current?.next_pos_order_num) || 0,
        );
        const issuedOnCart = Number(current?.pos_order_num) || 0;
        // next_pos_order_num / editOrderNo are the *next* ticket to issue — seed lastIssued = next−1.
        const lastIssuedHint = Math.max(issuedOnCart, uiNext > 0 ? uiNext - 1 : 0);
        if (lastIssuedHint > 0) {
          await seedLocalPosTicketSeq(lastIssuedHint, null, floatSessionId || null);
        }
      } catch {
        /* non-fatal */
      }
  ...
  async function commitCartLine({
      ...
      const nextLocal = await upsertLocalPosCartLine(
        {
          ...
          // Carry the ticket the cashier already sees (New Order - 32) through offline adds.
          ...(working?.pos_order_num != null && Number(working.pos_order_num) > 0
            ? {
                pos_order_num: Number(working.pos_order_num),
                pos_order_date: working.pos_order_date ?? null,
              }
            : {}),
          ...(working?.next_pos_order_num != null && Number(working.next_pos_order_num) > 0
            ? {
                next_pos_order_num: Number(working.next_pos_order_num),
                next_pos_order_date: working.next_pos_order_date ?? null,
              }
            : Number(editOrderNo) > 0
              ? {
                  next_pos_order_num: Number(editOrderNo),
                  next_pos_order_date: todayPosOrderDate(),
                }
              : {}),
        },
    ...
    try {
      if (persistedPatchRef) {
        ...
        if (intendedEdit && product?.product_code) {
          nextCart = preserveClientLineSkuAfterMutation(
  ...
  async function handleBarcodeEnter(code) {
    ...
    if (swapActive) {
      void pickProduct(product, { fromKeyboard: true });
      return true;
    }
    ...
    if (classicLayout) {
      await pickProduct(product, { fromKeyboard: true });
      return true;
    }
  ...
  function handleSwapDraftQtyChange(line, value) {
    ...
    if (!draft) return;
    const matches =
      cartLineMatchesRef(line, draft.lineId) ||
      cartLineMatchesRef(line, draft.line) ||
  ...
  async function pickProduct(product, opts = {}) {
    ...
    // Swap still needs retail package before drafting qty; Find/select must park first
    // so qty mounts even when retail_package fetch is slow or offline.
    if (isSwap) {
      await ensureRetailPackageForProduct(product);
    }
    ...
    if (isSwap) {
      ...
      const stableLineId = cartLineRef(replaceLine) ?? replaceLine.id;
      const nextSwapDraft = {
        lineId: stableLineId,
        line: replaceLine,
        product,
      ...
      replaceTargetSnapshotRef.current = {
        id: replaceLine.id,
        update_code: replaceLine.update_code ?? stableLineId,
  ...
  function beginReplaceCartLine(lineId) {
    ...
    swapDraftRef.current = null;
    setSwapDraft(null);
    const stableLineId = cartLineRef(line) ?? line.id;
    replaceTargetSnapshotRef.current = {
      id: line.id,
      update_code: line.update_code ?? stableLineId,
    ...
    replacingLineIdRef.current = stableLineId;
    setReplacingLineId(stableLineId);
    setSelectedLineId(stableLineId);
    setEditingLineId(null);
    setEditingLineRef(null);
    setSelectedProduct(null);
    selectedProductRef.current = null;
  ...
  async function replaceCartLineWithProduct(line, product, entryQty, discount = 0, override = null) {
    ...
    const replaceNeedle =
      ...
      line ??
      {
        ...
        update_code: replacingLineIdRef.current ?? swapDraftRef.current?.lineId,
        client_line_id: replacingLineIdRef.current ?? swapDraftRef.current?.lineId,
        product_code: line.product_code,
        on_wholesale_retail: line.on_wholesale_retail,
      };
    const liveLine =
      findCartLineForEdit(activeCart.lines, replaceNeedle, {
        preferProductCode: replaceNeedle.product_code ?? line.product_code,
      }) ??
    ...
    const nextLine = {
      ...
      ...(isPreviousOrderEditSession(activeCart) ? { _draftEdit: true } : {}),
    };
  ...
  // Optimistic TemporaryCart remints change line.id while swap still holds pending-*.
  // Heal replacingLineId / swapDraft so the classic row keeps showing the scan slot.
  useEffect(() => {
    const snap = replaceTargetSnapshotRef.current;
    const activeId = replacingLineIdRef.current;
    ...
    const needle =
      ...
      ({
        id: activeId,
        update_code: activeId,
        client_line_id: activeId,
      });
    const live = findCartLineForEdit(lines, needle, {
      preferProductCode: needle.product_code,
    });
    if (!live) return;
    const stable = cartLineRef(live) ?? live.id;
    replaceTargetSnapshotRef.current = {
      id: live.id,
      update_code: live.update_code ?? stable,
    ...
    if (String(stable) !== String(activeId)) {
      ...
      setReplacingLineId(stable);
    }
    const draft = swapDraftRef.current;
    if (
      draft &&
      (cartLineMatchesRef(live, draft.lineId) ||
        cartLineMatchesRef(live, snap) ||
        String(draft.lineId) === String(activeId))
    ) {
      if (
        String(draft.lineId) !== String(stable) ||
        !cartLineMatchesRef(draft.line, live.id)
      ) {
        const nextDraft = { ...draft, lineId: stable, line: live };
        swapDraftRef.current = nextDraft;
        setSwapDraft(nextDraft);
      }
    }
  }, [cart?.lines]);
  ...
  useEffect(() => {
    const onDown = (e) => {
      if (e.key === "Enter") enterKeyHeldRef.current = true;
    };
    const onUp = (e) => {
      if (e.key === "Enter") enterKeyHeldRef.current = false;
    };
  ...
  useEffect(() => {
    ...
    scheduleFocusEntryQty({ ignoreEnterMs: 0 });
    ...
    const replaceNeedle =
      ...
      (replacingLineIdRef.current
        ? {
            ...
            client_line_id: replacingLineIdRef.current,
          }
        : null);
    const replaceLine = replaceNeedle
      ? findCartLineForEdit(cartRef.current?.lines ?? cart?.lines, replaceNeedle, {
          preferProductCode: replaceNeedle.product_code,
        })
      : null;
  ...
  async function rebuildCart(remainingLines) {
    ...
    if (!isServerPosCartId(cart.id) || usesLocalPosCartWorkspace(cart)) {
      ...
      setCart(nextCart);
      if (isPreviousOrderEditSession(nextCart)) {
        await persistPreviousOrderLocalDraft(nextCart);
      }
      return;
    }
    // Previous-order edit: never DELETE /lines (clears markers → new order on checkout).
    // Replace-lines keeps held_order_num / superseded_sale_id.
    if (isPreviousOrderEditSession(cart)) {
      const linePayload = (remainingLines ?? [])
      ...
      const updated = await apiRequest(`/sales/carts/${cart.id}/lines`, {
        method: "PUT",
        body: {
          lines: linePayload,
      ...
      const withMarkers = {
        ...
        superseded_sale_id: cart.superseded_sale_id ?? merged.superseded_sale_id,
      };
      cartRef.current = withMarkers;
      setCart(withMarkers);
      await persistPreviousOrderLocalDraft(withMarkers);
      return;
    }
  ...
  function cartLineMatchesSelection(line, idSet) {
    ...
    return [...idSet].some(
      (id) =>
        sameLineId(line.id, id) ||
        sameLineId(line.update_code, id) ||
        sameLineId(line.client_line_id, id) ||
        sameLineId(cartLineRef(line), id),
    );
  }
  ...
  async function removeCartLinesByIds(rawIds) {
    ...
    const liveCart = cartRef.current ?? cart;
    if (!liveCart?.id || !liveCart?.lines?.length || !ids.length) {
      releaseCartLineMutationGate();
      return;
    }
    ...
    const isPreviousOrderEdit = Boolean(
      liveCart.held_order_num && liveCart.superseded_sale_id,
    );
  ...
  async function clearAllLines() {
    const liveCart = cartRef.current ?? cart;
    if (!liveCart?.id || !liveCart?.lines?.length) return;
    if (
      !(await confirm({
        title: "Clear cart",
        message: liveCart.held_order_num
          ? instantAutoEditSync
  ...
  async function handleCheckout(body, options = {}) {
    ...
    if (
      ...
      !activeCart.superseded_sale_id &&
      !activeCart.offline_client_sale_uuid &&
      (editSourceSale || activeCart.previous_order_edit)
    ) {
      setPaymentError(
  ...
  async function queuePreviousOrderEditOutboxNow({ force = false } = {}) {
    ...
    let cartNow = cartRef.current;
    if (!cartNow?.held_order_num) return null;
    if (isFreshWorkspacePlaceholder(cartNow)) return null;

    // Heal superseded_sale_id from the opened receipt before queuing — otherwise
    // sync can mint a new order_num instead of superseding.
    if (!cartNow.superseded_sale_id && editSourceSale?.id) {
      cartNow = {
        ...cartNow,
        superseded_sale_id: Number(editSourceSale.id),
        held_order_num:
          cartNow.held_order_num ?? editSourceSale.order_num ?? null,
      };
      cartRef.current = cartNow;
      setCart(cartNow);
    }
  ...
  function openSaveOrderDialog(mode) {
    ...
    // Park Scan immediately so product search cannot steal keys or open over the dialog.
    parkScanForOverlay();
  ...
  async function handlePrintPendingOfflineReceipts(orders) {
    ...
    if (!list.length) {
      notifyError("No pending offline receipts to print.");
      return;
    }
    ...
    const loadingToastId = toast.loading(
      total === 1
        ? "Printing pending receipt…"
        : `Printing ${total} pending receipts…`,
    );
    let printed = 0;
    let failed = 0;
    try {
      for (const sale of list) {
        ...
        try {
          ...
          if (total > 1) {
            toast.loading(
              orderLabel !== "—"
                ? `Printed ${printed} of ${total} (${orderLabel})…`
                : `Printed ${printed} of ${total}…`,
              { id: loadingToastId },
            );
          }
        } catch (e) {
          console.error("Pending offline receipt print failed", e);
          failed += 1;
        }
      }
      if (printed <= 0) {
        toast.error(
          failed > 0
            ? "Could not print pending receipts."
            : "Print cancelled or no format was selected.",
          { id: loadingToastId },
        );
        return;
      }
      toast.success(
        failed > 0
          ? `Printed ${printed} of ${total} pending receipt${total === 1 ? "" : "s"} (${failed} failed).`
          : total === 1
            ? "Printed pending receipt."
            : `Printed all ${printed} pending receipts.`,
        { id: loadingToastId },
      );
    } catch (e) {
      console.error("Pending offline receipt batch print failed", e);
      toast.error(e?.message ?? "Could not print pending receipts.", {
        id: loadingToastId,
      });
    }
  }
  ...
  function restoreEditOrderNoAfterFailedLoad({
    ...
    const prev = String(previousEditOrderNo ?? "").trim();
    // Searched for the blank next ticket (or a missing receipt with that #) —
    // keep that number. Never advance past a Cash Sales # that does not exist.
    if (prev && failedTicket != null && prev === String(failedTicket)) {
      setEditOrderNo(prev);
      orderNoUserEditedRef.current = false;
      return;
    }
    ...
    const candidates = [nextScoped, liveNext]
      .map((n) => Number(n))
      .filter((n) => Number.isFinite(n) && n > 0);
    // Prefer the prior blank next when it is still the first unused ticket
    // (e.g. stay on #1 after "No POS order found with Cash Sales #1").
    if (prev) {
      const prevNum = Number(prev);
      if (Number.isFinite(prevNum) && prevNum > 0) {
        const floor = candidates.length ? Math.min(...candidates) : 1;
        if (prevNum <= floor) {
          setEditOrderNo(prev);
          orderNoUserEditedRef.current = false;
          return;
    ...
    if (candidates.length) {
      setEditOrderNo(String(Math.min(...candidates)));
      orderNoUserEditedRef.current = false;
      return;
    }
    if (prev) {
    ...
    // Last resort: never leave a failed receipt search as a jumped blank next.
    setEditOrderNo("1");
    orderNoUserEditedRef.current = false;
  }
  ...
  async function restoreOrderForEdit(saleId, { replace = false, saleSnapshot = null, keepEditing = false } = {}) {
    ...
    try {
        ...
        // Outbox already uploaded (or removed after sync) — reopen the live server sale
        // instead of stranding the till on a blank workspace.
        const shouldHandoffToServer =
          /already synced/i.test(message) || /no longer in the local queue/i.test(message);
        if (shouldHandoffToServer) {
          const uuid = parseOfflineSaleUuid(saleId);
          let serverSaleId = null;
          try {
            const row = uuid ? await idbGetOutboxSale(uuid) : null;
            serverSaleId =
              row?.server_sale_id != null && Number(row.server_sale_id) > 0
                ? Number(row.server_sale_id)
                : null;
            if (!serverSaleId) {
              const ticket =
          ...
          if (serverSaleId) {
            handoffToServerSale = true;
            await restoreOrderForEdit(serverSaleId, {
              replace: true,
              saleSnapshot: saleSnapshot?.id ? saleSnapshot : null,
            });
            return;
          }
        }
  ...
  async function openCompletePayment() {
    ...
    const run = (async () => {
      ...
      // Clear stuck paint/busy that used to flash "Still saving cart lines" forever.
      if (lineBusyRef.current && cartCommitPendingRef.current === 0) {
        lineBusyRef.current = false;
        setLineBusy(false);
      }
      let activeCart = healPreviousOrderEditMarkers(cartRef.current ?? cart);
  ...
  posShortcutStateRef.current = {
    ...
    cartLineSaveWaitBusy,
    getCartLineSaveBlocking: () => isCartLineSaveBlocking(),
  };
  ...
  useEffect(() => {
    ...
    function isModalOpen(state) {
      return (
        ...
        || Boolean(state.getCartLineSaveBlocking?.())
        || isConfirmDialogOpen()
      );
    }
    ...
    function onPosKeyEvent(e, phase) {
      ...
      if (classicDelete) {
        ...
        e.__centrixPosShortcutHandled = true;
        actions.closeProductSearchDropdown?.();
        // Arm before the async remove starts — Delete→Enter used to twin the next add.
        actions.armCartLineMutationGate?.();
                  ...
                  <button
                    ...
                    title={
                      failedSyncOrders.length > 0
                        ? `${failedSyncOrders.length} offline order(s) failed to sync — open to retry`
                        : `${pendingSync} offline order(s) waiting to sync`
                    }
                    onClick={() => setPendingSyncOpen(true)}
                    className={posHeaderBtnClassName}
                  >
                    <span className="pos-header-btn-label" data-short="Sync">
                      {failedSyncOrders.length > 0 ? "Sync failed" : "Pending sync"}
                    </span>
                    <span
                      className={`pos-header-action-badge${
                        failedSyncOrders.length > 0 ? " bg-amber-500" : ""
                      }`}
                    >
                      {(failedSyncOrders.length > 0
      ...
      >
        ...
        <div className="pos-left-panel flex min-h-0 w-full flex-col self-stretch border-b border-[var(--theme-border)] bg-[var(--theme-page-bg)] lg:w-[min(100%,28rem)] lg:shrink-0 lg:border-b-0 lg:border-r xl:w-[32rem]">
          <div className="pos-search-panel shrink-0 border-b border-[var(--theme-border)] px-4 py-2.5">
            ...
            {standalone && (offlineMode || failedSyncOrders.length > 0 || pendingSync > 0) ? (
              <div
                className={`mt-2 rounded-md border px-2.5 py-2 text-xs font-medium ${
                  offlineMode || failedSyncOrders.length > 0
