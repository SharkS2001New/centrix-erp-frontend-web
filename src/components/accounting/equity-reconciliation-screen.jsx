...
export function EquityReconciliationScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await apiRequest("/accounting/equity-reconciliation");
      setData(res);
      if (res?.enabled === false) {
        setDisabledMessage(res.message ?? "Equity reconciliation is not enabled.");
      }
    } catch (e) {
      notifyError(e instanceof ApiError ? e.message : "Failed to load Equity reconciliation");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const loadDetails = useCallback(async (paymentId) => {
    ...
    try {
      const res = await apiRequest(`/accounting/equity-reconciliation/${paymentId}`);
  ...
  const applyPayment = async (paymentId, saleId) => {
    setBusyId(paymentId);
    try {
      await apiRequest(`/accounting/equity-reconciliation/${paymentId}/apply`, {
        method: "POST",
        body: { sale_id: saleId },
      });
  ...
  const ignorePayment = async (payment) => {
    ...
    try {
      await apiRequest(`/accounting/equity-reconciliation/${payment.id}/ignore`, {
        method: "POST",
        body: {},
      });
    ...
    >
      {disabledMessage ? (
        <div className="rounded-xl border border-slate-200 bg-slate-50 px-6 py-8 text-center">
          ...
          <p className="mt-3 text-sm text-slate-600">
            Turn on <strong>Enable Equity paybill reconciliation</strong> under Admin → Equity Bank accounts, then save.
          </p>
          <Link
            href="/admin/settings"
      ...
      <div className="mb-6 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">
        ...
        <p className="mt-1 text-xs text-sky-800">
          Enable or change this under Admin → Equity Bank accounts.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-200 bg-white px-6 py-10 text-center text-sm text-slate-500">
          No unmatched Equity payments right now.
        </div>
      ) : (
