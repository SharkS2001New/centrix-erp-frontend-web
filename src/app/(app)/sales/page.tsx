...
import type { Sale, TemporaryCart } from "@/types/api";
...
export default function SalesPage() {
  ...
  const [cart, setCart] = useState<TemporaryCart | null>(null);
  ...
  async function createCart() {
    ...
    setSale(null);
    try {
      const c = await apiRequest<TemporaryCart>("/sales/carts", {
        method: "POST",
        body: {
          channel,
  ...
  async function addLine(e: FormEvent) {
    ...
    try {
      ...
      const updated = await apiRequest<TemporaryCart>(`/sales/carts/${cart.id}`);
      setCart(updated);
  ...
  async function checkout() {
    ...
    setMessage(null);
    try {
      const s = await apiRequest<Sale>(`/sales/carts/${cart.id}/checkout`, {
        method: "POST",
        body: {
          status: "completed",
