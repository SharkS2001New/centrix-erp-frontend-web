...
function AlertNotificationConfigSummary() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiRequest("/admin/system-issue-alert-settings", { loading: false });
      setSettings(res.settings ?? res.data?.settings ?? {});
    } catch (e) {
  ...
  const digestOn = settings?.email_digest_enabled !== false;
  const digestEmail = String(settings?.digest_email ?? "").trim();
  const instantEmailOn = Boolean(settings?.instant_email_enabled);
  const whatsappOn = Boolean(settings?.whatsapp_instant_enabled);
  const whatsappNumber = String(settings?.whatsapp_number ?? "").trim();
  ...
  return (
    <section className="theme-panel mb-4 rounded-xl border p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Delivery is configured under{" "}
            <Link
              href="/platform/settings?tab=alerts"
              className="font-medium text-[#185FA5] hover:underline"
            >
              Platform settings → Alert notifications
            </Link>
            . Email uses platform email delivery; WhatsApp uses platform WhatsApp credentials.
          </p>
        </div>
      ...
      {loading && !settings ? (
        <p className="mt-3 text-sm text-slate-500">Loading current configuration…</p>
      ) : (
        <ul className="mt-3 space-y-1.5 text-sm text-slate-700 dark:text-slate-200">
          <li>
            ...
            {digestOn
              ? digestEmail
                ? `On — ${digestEmail}`
                : "On — no digest email set"
              : "Off"}
          </li>
          <li>
            <span className="font-medium">Instant email (high-priority / new / user reports):</span>{" "}
            {instantEmailOn ? (digestEmail ? `On — ${digestEmail}` : "On — no email set") : "Off"}
          </li>
          <li>
            <span className="font-medium">Instant WhatsApp:</span>{" "}
            {whatsappOn
              ? whatsappNumber
                ? `On — ${whatsappNumber}`
                : "On — no number set"
...
/** First time this fingerprint was recorded (across matching occurrences). */
function issueFirstSeen(row) {
  return row.first_seen_at ?? row.created_at ?? null;
}

/** Last time this fingerprint occurred; falls back to first seen when missing. */
...
export default function PlatformSystemIssuesPage() {
  ...
  return (
    ...
    >
      ...
      <AlertNotificationConfigSummary />
          ...
          <div className="theme-modal max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border p-6 shadow-2xl">
            ...
            <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
              ...
              <div>
                ...
                <dd>
                  {formatWhen(issueLastSeen(selected))}
