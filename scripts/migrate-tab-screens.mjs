...
`;
  ...
  const usedLocals = new Map();
  function uniqueLocal(name, id) {
    if (!usedLocals.has(name)) {
      usedLocals.set(name, id);
      return name;
    }
    const alt = `${toPascalCase(id)}`;
    usedLocals.set(alt, id);
    return alt;
  }
  ...
  for (const e of entries) {
    if (e.exportName === "default") {
      const local = uniqueLocal(toPascalCase(e.id) || "Screen", e.id);
      importLines.push(`import ${local} from "${e.importPath}";`);
      mapLines.push(`  ${JSON.stringify(e.id)}: ${local},`);
    } else {
      const local = uniqueLocal(e.exportName, e.id);
      if (local === e.exportName) {
        importLines.push(`import { ${e.exportName} } from "${e.importPath}";`);
      } else {
        importLines.push(
          `import { ${e.exportName} as ${local} } from "${e.importPath}";`,
        );
      }
      mapLines.push(`  ${JSON.stringify(e.id)}: ${local},`);
    }
