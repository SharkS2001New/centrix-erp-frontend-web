...
Required repo secrets for **R2 hosting** (same as MySQL backup R2): `BACKUP_R2_ACCESS_KEY_ID`, `BACKUP_R2_SECRET_ACCESS_KEY`, `BACKUP_R2_BUCKET`, `BACKUP_R2_ENDPOINT`, `BACKUP_R2_PUBLIC_URL` (plus `PERSONAL_ACCESS_TOKEN` for Helm sync). MSI objects use prefix `print-agent/` in that bucket. If those secrets are missing, CI still builds the MSI and publishes a GitHub Release; R2 upload is skipped.
